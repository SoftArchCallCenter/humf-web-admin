...
import { getTickets, updateTicket } from "@/logic/kitchen";
    ...
    getTickets(resId).then((result) => {
      // console.log(result)
			if(result.err){
				...
				setTicketList(result.result.tickets)
			}
		})
	},[])
                ...
                <li key= {ticket.id}>
                  <div className="px-4 py-5 sm:px-6 border-2 border-b-slate-300">
                    <div className="flex items-center justify-between">
                      ...
                      <a className="font-medium text-blue-600 hover:text-blue-800" onClick={() => handleFinish(ticket.id)}>Finish</a>
                    </div>
                    <ul className="text-gray-600 list-disc ml-6">
                      {ticket.order.menus.map( (menu, index) => {
                        return (<li className="text-sm"> Menu {index+1} : {menu.name} <br/> Quatity : {menu.quatity}</li>)
                      })}
                    </ul>
                  </div>
                </li>
